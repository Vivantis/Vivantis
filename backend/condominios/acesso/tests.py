...
from condominios.models import Condominio, Unidade, Morador, Visitante, Prestador, ControleAcesso
...
@pytest.fixture
def setup_acesso(db):
    condominio = Condominio.objects.create(nome="Residencial Teste", endereco="Rua Exemplo")
    unidade = Unidade.objects.create(numero="101", bloco="A", condominio=condominio)
    morador = Morador.objects.create(nome="João", email="joao@email.com", unidade=unidade)
    visitante = Visitante.objects.create(nome="Carlos", documento="123456789", unidade=unidade)
    prestador = Prestador.objects.create(nome="Limpeza LTDA", servico="Limpeza", condominio=condominio)
    return {"unidade": unidade, "morador": morador, "visitante": visitante, "prestador": prestador}
...
def test_filtrar_por_tipo(cliente_autenticado, setup_acesso):
    ...
    assert response.status_code == 200
    for acesso in response.data["results"]:
